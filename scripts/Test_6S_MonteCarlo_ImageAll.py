...
def statisticsImage(fileName):
	...
	for band in range( src_ds.RasterCount ):
	    ...
	    if srcband is None:
		continue
	    bands = srcband.ReadAsArray() 
	    median = numpy.median(bands)	
	    stats = srcband.GetStatistics( True, True )
	    stats.append(median) 	
	    if stats is None:
		continue
	    statss.append(stats)	
	    print "[ STATS ] =  Minimum=%.3f, Maximum=%.3f, Mean=%.3f, StdDev=%.3f, Median=%.3f" % ( \
		        stats[0], stats[1], stats[2], stats[3], stats[4])
	return statss
def executionParameters6SusingARCSIls8(inputHeader,sensor,radianceImage,TOAImage,outpath,OutName):
	...
	cont = 0
	with open(outpath+'/'+OutName+'_Atmosphericparameters.csv', 'wb') as csvfile:
	    	fieldnames = ['atmoProfile', 'aeroProfile','GroundReflectance','AOT','band','aX', 'bX', 'cX','MeanSRRAD','MaxSRRAD','MinSRRAD','StdSRRAD','MedianSRRAD','MeanSRTOA','MaxSRTOA','MinSRTOA','StdSRTOA','MedianSRTOA','ResultsRADMin', 'ResultsRADMax','ResultsRADMean','ResultsRADMedian','ResultsTOAMin','ResultsTOAMax','ResultsTOAMedian','ResultsTOAMean', 'MeanRAD','MaxRAD','MinRAD','StdRAD','MedianRAD','MeanTOA','MaxTOA','MinTOA','StdTOA','MedianTOA']
		...
		for paramAtmProfile in [AtmosProfile.Tropical, AtmosProfile.MidlatitudeSummer, AtmosProfile.MidlatitudeWinter, AtmosProfile.SubarcticSummer,AtmosProfile.SubarcticWinter, AtmosProfile.NoGaseousAbsorption]:
			...
			print('-------------------------------------------------------')
			for paramAeroProfile in[AeroProfile.Continental, AeroProfile.NoAerosols ,AeroProfile.Maritime,AeroProfile.Urban,AeroProfile.Desert,AeroProfile.BiomassBurning,AeroProfile.Stratospheric]:
				...
				for paramGroundReflect in [GroundReflectance.GreenVegetation, GroundReflectance.ClearWater, GroundReflectance.Sand,GroundReflectance.LakeWater]:
						...
						for aotvalue in numpy.linspace(0,1,11):
							...
							print(aotvalue)
							print('-------------------------------------------------------')
							outputImage = os.path.join(outpath, outSRFName+'-RAD-'+str(paramAtmProfile)+'-'+str(paramAeroProfile)+'-'+str(paramGroundReflect)+'-'+str(aotvalue)+'.kea')
							outputImageTOA = os.path.join(outpath, outSRFName+'-TOA-'+str(paramAtmProfile)+'-'+str(paramAeroProfile)+'-'+str(paramGroundReflect)+'-'+str(aotvalue)+'.kea')
							imgBandCoeffs = list()
							...
							rsgislib.imagecalibration.apply6SCoeffSingleParam(TOAImage, outputImageTOA, outFormat, rsgislib.TYPE_16UINT, 1000, 0, True, imgBandCoeffs)	
							statSRRAD = statisticsImage(outputImage)
							statSRTOA = statisticsImage(outputImageTOA)
#							os.remove(outputImage)
							for band in imgBandCoeffs:
							    ...
							    ymedian = ((band.aX * statRAD[band.band - 1][2]) - band.bX)
							    bias = band.cX * ymedian			 
							    resultsRadMedian = ymedian/(1 + bias)
							    ...
							    ymedian = ((band.aX * statTOA[band.band - 1][4]) - band.bX)
							    bias = band.cX * ymedian	 
							    resultsToaMedian = ymedian/(1 + bias) 
							    writer.writerow({'atmoProfile': str(paramAtmProfile), 'aeroProfile': str(paramAeroProfile),'GroundReflectance':str(paramGroundReflect),'AOT':str(aotvalue),'band':str(band.band),'aX':str(band.aX),'bX':str(band.bX),'cX':str(band.cX),'MeanSRRAD':str(statSRRAD[band.band - 1][2]),'MinSRRAD':str(statSRRAD[band.band - 1][0]),'MaxSRRAD':str(statSRRAD[band.band - 1][1]),'StdSRRAD':str(statSRRAD[band.band - 1][3]),'MedianSRRAD':str(statSRRAD[band.band - 1][4]),'MeanSRTOA':str(statSRTOA[band.band - 1][2]),'MinSRTOA':str(statSRRAD[band.band - 1][0]),'MaxSRTOA':str(statTOA[band.band - 1][1]),'StdSRTOA':str(statTOA[band.band - 1][3]),'MedianSRTOA':str(statTOA[band.band - 1][4]),'ResultsRADMin':str(resultsRadMin),'ResultsRADMax':str(resultsRadMax),'ResultsRADMean':str(resultsRadMean),'ResultsRADMedian':str(resultsRadMedian),'ResultsTOAMin':str(resultsToaMin),'ResultsTOAMax':str(resultsToaMax),'ResultsTOAMean':str(resultsToaMean),'ResultsTOAMedian':str(resultsToaMedian),'MeanRAD':str(statRAD[band.band - 1][2]),'MinRAD':str(statRAD[band.band - 1][0]),'MaxRAD':str(statRAD[band.band - 1][1]),'StdRAD':str(statRAD[band.band - 1][3]),'MedianRAD':str(statRAD[band.band - 1][4]),'MeanTOA':str(statTOA[band.band - 1][2]),'MinTOA':str(statTOA[band.band - 1][0]),'MaxTOA':str(statTOA[band.band - 1][1]),'StdTOA':str(statTOA[band.band - 1][3]),'MedianTOA':str(statTOA[band.band - 1][4])})
							    print(band)	
							
							print(outputImage)
							os.remove(outputImage)
							os.remove(outputImageTOA)
							cont = cont + 1

		print(cont)

##
def executionParameters6SusingARCSI(inputHeader,sensor,radianceImage,TOAImage,outpath,OutName):
	#inputHeader = '/home/juanmolina/Documents/6s/LE70080592013246EDC00/LE70080592013246EDC00_MTL.txt';
	wktStr = None
	#sensor = 'ls7'
	imagefile = None
...
if __name__ == '__main__':
	...
	if sensor == "ls8":
		executionParameters6SusingARCSIls8(mtl,sensor,radImage,TOAImage,outpath,OutName)
		print("--- %s seconds ---" % (time.time() - start_time))		
	else:
		executionParameters6SusingARCSI(mtl,sensor,radImage,TOAImage,outpath,OutName)	
		print("--- %s seconds ---" % (time.time() - start_time))
	print("--- %s seconds ---" % (time.time() - start_time))
