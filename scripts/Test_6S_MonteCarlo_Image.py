...
def executionParameters6SusingARCSIls8(inputHeader,sensor,radianceImage,TOAImage,outpath,OutName):
	...
	with open(outpath+'/'+OutName+'_Atmosphericparameters.csv', 'wb') as csvfile:
	    	fieldnames = ['atmoProfile', 'aeroProfile','GroundReflectance','AOT','band','aX', 'bX', 'cX','MeanSRRAD','MaxSRRAD','MinSRRAD','StdSRRAD','MeanSRTOA','MaxSRTOA','MinSRTOA','StdSRTOA']
...
#							os.remove(outputImage)
							for band in imgBandCoeffs:
							    writer.writerow({'atmoProfile': str(paramAtmProfile), 'aeroProfile': str(paramAeroProfile),'GroundReflectance':str(paramGroundReflect),'AOT':str(aotvalue),'band':str(band.band),'aX':str(band.aX),'bX':str(band.bX),'cX':str(band.cX),'MeanSRRAD':str(statSRRAD[band.band - 1][2]),'MinSRRAD':str(statSRRAD[band.band - 1][0]),'MaxSRRAD':str(statSRRAD[band.band - 1][1]),'StdSRRAD':str(statSRRAD[band.band - 1][3]),'MeanSRTOA':str(statSRTOA[band.band - 1][2]),'MinSRTOA':str(statSRRAD[band.band - 1][0]),'MaxSRTOA':str(statTOA[band.band - 1][1]),'StdSRTOA':str(statTOA[band.band - 1][3])})
							    print(band)	
